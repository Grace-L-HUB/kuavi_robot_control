...
RETREAT_BACK_M = 0.08
RETREAT_LIFT_Z = 0.05
# 夹紧后：先明显上提（夹持）→ 水平后撤 → 松爪 → 回零
POST_GRASP_HORIZONTAL_RELEASE = True
POST_GRASP_LIFT_DURATION_S = 4.5
POST_GRASP_LIFT_HOLD_S = 1.5
...
def resolve_grasp_poses_arm_base(
    ...
    release_retreat = (grasp[0] - rel_back, grasp[1], lift[2])
...
def _solve_ik(ik_proxy, pos, hand: str, *, use_prev_q0: bool = True):
...
def _apply_grasp_coordinates(camera_point: tuple, hand: str) -> None:
    ...
    global GRASP_POS, RELEASE_RETREAT, LIFT_POS, RETREAT
    ...
    if POST_GRASP_HORIZONTAL_RELEASE:
        _log(f"[坐标] 夹紧后上提 (m): {LIFT_POS}")
        _log(f"[坐标] 上提后水平后撤放开 (m): {RELEASE_RETREAT}")
...
def run_grasp(hand: str, grasp_width: int, grasp_effort: float,
    ...
    if USE_THREE_STAGE_APPROACH:
        _log("[策略] 三段位姿：左上方 → 平移至正上方 → 垂直下降抓取 → 闭合 → 上提 → 水平后撤放开 → 回零")
        _log(
    ...
    if POST_GRASP_HORIZONTAL_RELEASE:
        _log(f"[策略] 放开：先上提 {POST_GRASP_LIFT_DURATION_S}s，保持 {POST_GRASP_LIFT_HOLD_S}s，再水平后撤 {RELEASE_RETREAT_BACK_M}m 后松爪")
    ...
    if POST_GRASP_HORIZONTAL_RELEASE:
        _log("========== 夹紧后上提（保持夹持） ==========")
        if not _move_to(
            ik_proxy, arm_pub, LIFT_POS, hand, POST_GRASP_LIFT_DURATION_S,
            "夹紧后上提", use_target_poses,
        ):
            return False
        _log(f"[运动] 上提到位，保持 {POST_GRASP_LIFT_HOLD_S}s ...")
        rospy.sleep(POST_GRASP_LIFT_HOLD_S)
        _log("========== 上提后水平后撤 ==========")
        if not _move_to(
